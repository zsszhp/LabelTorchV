...
* **验证规则**：所有的归一化坐标必须满足 $0.0 \le x, y, w, h \le 1.0$。若越界，平台在扫描时自动发出警告，并在导入时限制归入异常池。
...
* **缩放 (Zoom)**：支持以鼠标当前悬浮点为中心，进行滚轮缩放，步长为 $10\%$。
...
* **PRO (Principal Radial Orientation / Per-Region Overlap)**：按缺陷区域大小加权的重叠率。能更客气地评价微小 defects 的检出率。

### 3. 可视化图表
* **PR 曲线图**：使用折线图绘制不同置信度阈值下的 Precision 与 Recall 走向，并在交叉点标注最佳阈值推荐。
* **混淆矩阵热力图 (Confusion Matrix)**：展示类别之间的误检关系（如：将“划痕”误判为“灰尘”）。行表示真实标签，列表示预测标签，对角线归一化高亮。
* **推理热力图 (Heatmap)**：
...
├── LabelTorch.exe                 # Electron/C++ 宿主主程序
├── resources/                     # 静态资源与前端构建包
...
```mermaid
graph TD
    A[启动 LabelTorch.exe] --> B[拉起后台计算进程 runtime/python/python.exe]
    B --> C{调用 PyTorch 测试 CUDA}
    C -->|显卡可用 & Driver 匹配| D[日志记录 GPU 型号及可用显存]
    C -->|显卡不可用/未装驱动| E[弹出“未检测到可用GPU驱动，自动切换至CPU降级模式”警告]
    D --> F[宿主与子进程建立 JSON-RPC 管道]
    E --> F
    F --> G[进入主页面]
```

* **显卡自检命令**：后端自动在 Python 中运行以下脚本，将结果输出到 IPC：
  ```python
  import torch
...
* **继承加载**：新建训练任务时，支持选择历史模型权重（例如 `models/v1.0_best.pt`）作为初始权重，继承已有的特征提取能力。
* **数据快照对齐**：通过锁定数据快照 `Snapshot`，每次增量训练都会在数据库中建立从属关系：
